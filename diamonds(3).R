...
#Model 1 is not transformed.
x<-diamonds$Size
y<-diamonds$Price
m1 <- lm(y ~ x)

par(cex.lab=1.4, cex.axis=1.4, pch=19, mfrow=c(1,1), mar=c(5,5,1,1))
...
#Prediction interval by hand, if you like:
...
par(cex.lab=1.4, cex.axis=1.4, pch=19, mfrow=c(2,2), mar=c(5,5,1.5,1))
plot(m1)
...
#Try a log transformation: Model 2.
x<-log(diamonds$Size)
...
#Correction factor confidence interval
anova(m2)  #Be careful what you put into the predict function when x is transformed!!!!
...
#Correction factor so tiny!  



#Use Box-Cox to choose a transformation: Call this Model 3.
...
bcy <- powerTransform(m3a)  #BCY is Box-Cox for the y-variable.  
summary(bcy)
...
abline(h=(4/49), lty=2)
#I'm not impressed.  But here's how you would do the correction factor:

predict(m3, interval="confidence", newdata=data.frame(invsize=c((1/0.25), (1/0.3))), level=0.95)

(1/(0.03819130^2 + 0.0000029)) * (1 + (2*0.0000029^2 + 4*0.03819130^2*0.0000029)/(0.03819130^2 + 0.0000029)^2) 
(1/(0.03957277^2 + 0.0000029)) * (1 + (2*0.0000029^2 + 4*0.03957277^2*0.0000029)/(0.03957277^2 + 0.0000029)^2) 
...
y.hat1.4 - t.val * sqrt(var.4 + sefit^2) #It assumed the variance of the individual at this value of x is the same as every other value.
y.hat1.4 + t.val * sqrt(var.4 + sefit^2)

#What we should be doing instead - divide var.4 by the weight for x*:
y.hat1.4 - t.val * sqrt(var.4*0.25 + sefit^2) #Remember the variance is weighted.
y.hat1.4 + t.val * sqrt(var.4*0.25 + sefit^2)
#Note that sometimes the interval will be wider, sometimes narrower than computed above.
